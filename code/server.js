...
app.post('/create-donation-order', async (req, res) => {
  try {
    const { variantId, email, fullName, phoneNumber, amount, slug, affiliateId } = req.body;
    ...
    // Check if the slug is a full domain URL
    if (slug.startsWith('http://') || slug.startsWith('https://')) {
        baseUrl = `${slug}/checkout/${variantId}:1`;
    } else {
        baseUrl = `https://${slug}.mycartpanda.com/checkout/${variantId}:1`;
    }

    // Base checkout URL with query parameters
    let checkoutUrl = `${baseUrl}?email=${encodedEmail}&first_name=${encodedFirstName}&last_name=${encodedLastName}&phone=${encodedPhoneNumber}`;

    // Add affiliate ID if it exists
    if (affiliateId) {
        checkoutUrl += `?afid=${encodeURIComponent(affiliateId)}`;
    }
